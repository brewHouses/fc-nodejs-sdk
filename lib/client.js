...
class Client {
  ...
  createFunction(serviceName, options, headers) {
    ...
    return this.post(`/inner/services/${serviceName}/functions/grantTempCodeBucketPermission`, getTmpOssTokenOptions, headers)
      .then(resp => this.uploadCodeToTmpOssBucket(this, resp, serviceName, options))
      .then(resp => this.largeFunctionHelper(this, resp, serviceName, options, headers, true));
  }
   ...
   */
  getTempCodeBucketPermission(serviceName, functionName, instanceType, headers) {
    var getTmpOssTokenOptions = {
  ...
  largeFunctionHelper(that, resp, serviceName, options, headers, isCreate, functionName) {
  ...
  uploadCodeToTmpOssBucket(that, resp, serviceName, options) {
    // TODO: 判断 httpStatus 是不是 403
    let fileSize = fs.statSync(options.code.zipFile).size;
    if (fileSize > resp.data.objectSizeLimit) {
      throw new Error(`the size of file ${fileSize} could not greater than ${resp.data.objectSizeLimit}`);
    }
    ...
    async function put () {
      try {
        await client.put(`${that.accountid}/${serviceName}/${resp.data.objectName}`, path.normalize(options.code.zipFile));
      } catch (e) {
        console.log(e);
      }
      return resp;
  ...
  updateFunction(serviceName, functionName, options, headers) {
    ...
    return this.post(`/inner/services/${serviceName}/functions/grantTempCodeBucketPermission`, getTmpOssTokenOptions, headers)
      .then(resp => this.uploadCodeToTmpOssBucket(this, resp, serviceName, options))
      .then(resp => this.largeFunctionHelper(this, resp, serviceName, options, headers, false, functionName));
  }
